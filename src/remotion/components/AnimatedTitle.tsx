  ...
  const getEnterValues = () => {
    ...
    switch (enterAnimation) {
      ...
      case "slideLeft":
        return {
          opacity,
          transform: `translateX(${interpolate(enterProgress, [0, 1], [80, 0])}px)`,
          filter: "none",
        };
      case "slideRight":
        return {
      ...
      case "blur": {
        const blur = interpolate(frame, [0, enterDuration], [20, 0], {
          extrapolateRight: "clamp",
        });
  ...
  const getExitValues = () => {
    ...
    switch (exitAnimation) {
      case "slideUp":
        return {
          opacity,
          transform: `translateY(${interpolate(exitProgress, [0, 1], [0, -60])}px)`,
          filter: "none",
        };
      case "slideDown":
        return {
      ...
      case "scale":
        return {
          opacity,
          transform: `scale(${interpolate(exitProgress, [0, 1], [1, 0.5])})`,
          filter: "none",
        };
      case "blur": {
  ...
  const displayText =
    enterAnimation === "typewriter" && frame < enterDuration
      ? text.slice(
          0,
          Math.floor(
            interpolate(frame, [0, enterDuration], [0, text.length], {
              extrapolateRight: "clamp",
            }),
          ),
        )
